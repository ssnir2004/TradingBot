...
# deploy/ibgateway-paper.service and deploy/ibgateway-live.service, each
# passing its own mode as $1.
#
# Paper and live run as two separate, simultaneous IB Gateway processes —
# each needs its own settings dir (so their login sessions don't clobber
# each other), its own log dir, and its own copy of IBC's gatewaystart.sh
# (IBC's gatewaystart.sh does not read environment variables — it has a
# block of `VAR=value` assignments at the top of the file that's meant to
# be edited directly; two instances patching the SAME physical file at
# once would race each other, so each mode gets its own copy instead).
set -euo pipefail
...
IBC_INI="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/config-${MODE}.ini"
TWS_SETTINGS_PATH="/home/tradingbot/ibgateway-settings-${MODE}"
LOG_PATH="/home/tradingbot/ibc-logs-${MODE}"
GATEWAYSTART_TEMPLATE="$IBC_PATH/gatewaystart.sh"
GATEWAYSTART="$IBC_PATH/gatewaystart-${MODE}.sh"
